...
try {
  ...
  for (const plan of p.plans) {
    const request = buildRequest(
      p,
      `demo-r${p.revision}-${plan.shot_id}`,
      plan.shot_id,
    );
    ...
    for (
      let i = 0;
      i < 5 &&
      !["succeeded", "needs_review", "failed", "submission_unknown"].includes(
        job.status,
      );
      i++
    )
  ...
  const assembly = process.env.DRAMAPILOT_FIXTURE_DIR
    ...
    : {
        status: "not_run",
        reason:
          "Set DRAMAPILOT_FIXTURE_DIR to generated B color/tone clips for local video assembly",
      };
