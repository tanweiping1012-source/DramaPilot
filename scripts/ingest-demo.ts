...
if (isAbsolute(runRelative) || runRelative === ".." || runRelative.startsWith("../")) throw new Error("Demo output must stay inside workspace");
...
const evidence = (id: string, text: string, inferred = false) => {
  const start = script.indexOf(text);
  if (start < 0) throw new Error("Demo evidence excerpt missing");
