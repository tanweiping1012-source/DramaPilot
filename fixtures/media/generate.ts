...
import { defaultTools, runMedia, probe, concatenateFixture } from '../../src/media/local.ts';
import { resolveFrames, renderSrt } from '../../src/media/timeline.ts';
...
if (!/^[a-zA-Z0-9_/. -]+$/.test(font)) throw new Error('Unsupported fixture font path');
const clips = [];
for (const shot of shots) {
  ...
  if (Number(video?.nb_read_frames) !== shot.frames) throw new Error(`Unexpected encoded frames: ${shot.shotId}`);
...
const finalVideo = final.streams.find((s: any) => s.codec_type === 'video');
if (Number(finalVideo?.nb_read_frames) !== 175) throw new Error('Unexpected final frame count');
