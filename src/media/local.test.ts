...
import { mediaHash, mixAudioStems } from './local.ts';
test('streamed hash is actual content, stable for same bytes', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'dp-hash-'));
  try {
    const file = join(dir, 'data'); await writeFile(file, 'abc');
    assert.equal(await mediaHash(file), 'ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad');
    await writeFile(file, 'abcd'); assert.notEqual(await mediaHash(file), 'ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad');
...
test('invalid mix options fail before filesystem/process access', async () => {
  ...
  for (const patch of [{frames:0},{fps:0},{backgroundGain:-1},{backgroundGain:NaN}]) await assert.rejects(mixAudioStems({...input,...patch}), /Invalid/);
});
test('assembly and mix respect pre-aborted signals before input access', async () => {
  const { concatenateFixture } = await import('./local.ts');
...
test('runMedia abort stops an actual running child', async () => {
  const { runMedia } = await import('./local.ts');
  ...
  try {
    let pid = 0;
    for (let i = 0; i < 200; i++) { try { pid = Number(await readFile(pidFile, 'utf8')); break; } catch { await delay(10); } }
    assert.ok(pid > 0, 'child reported ready');
    controller.abort(); await rejected;
    let alive = true;
    for (let i = 0; i < 200; i++) { try { process.kill(pid, 0); await delay(10); } catch { alive = false; break; } }
    assert.equal(alive, false, 'cancelled child exited');
  } finally { controller.abort(); await rejected; await rm(directory, { recursive: true }); }
});
