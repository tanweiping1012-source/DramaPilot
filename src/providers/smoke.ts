...
import { concatenateFixture, mixAudioStems } from '../media/local.ts';
import { resolveFrames, renderSrt } from '../media/timeline.ts';
import type { Artifact } from '../contracts/index.js';
const fixtureDirectory = process.argv[2], target = process.argv[3];
if (!fixtureDirectory || !target) throw new Error('Usage: node dist/src/providers/smoke.js <B fixture directory> <new output directory>');
const directory = resolve(target); await mkdir(directory); // Refuse accidental rerun/overwrite.
...
for (const plan of familyFixture.plans) {
  ...
  if (result.status !== 'accepted') throw new Error('Smoke submit failed');
  for (let i = 0; i < 3; i++) await provider.poll(result.job_id, signal);
  const collected = await provider.collect(result.job_id, signal);
  if (collected.status !== 'collected' || !collected.artifacts[0]?.probe) throw new Error('Missing probed fixture');
...
if (diagnostics.submitCount !== 3) throw new Error('Unexpected submit count');
