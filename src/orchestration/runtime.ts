...
export async function createRuntime(options: {
  ...
  return {
    ...
    async review() {
      ...
      const report = await exportReview(
        path,
        readAdaptation(store),
        store.list(),
      );
      return { path, report };
    },
    async assemble(signal: AbortSignal) {
      ...
      const artifacts = p.plans.map((plan) => {
        const found = all
          .filter(
            (j) =>
              j.request.shot_id === plan.shot_id && j.status === "needs_review",
          )
      ...
      const output = join(root, `fixture-episode-r${p.revision}.mp4`),
        manifest = join(root, `episode-r${p.revision}.json`);
      try {
        const old = JSON.parse(await readFile(manifest, "utf8"));
        if (
          canonicalHash(old.episode) !== canonicalHash(episode) ||
      ...
      const temporary = join(
        root,
        `fixture-r${p.revision}-${randomUUID()}.mp4`,
      );
      try {
        const media = await concatenateFixture(
          artifacts.map((a) =>
            a.uri.startsWith("file:") ? fileURLToPath(a.uri) : a.uri,
          ),
          temporary,
          defaultTools,
          signal,
        ...
        await runMedia(
          defaultTools.ffmpeg,
          ["-nostdin", "-v", "error", "-i", temporary, "-f", "null", "-"],
          signal,
        );
        const srt = renderSrt(
          timeline,
