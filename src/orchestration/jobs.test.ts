...
function worker(
  ...
  return new Promise((resolve) => {
    const p = spawn(
      process.execPath,
      [fileURLToPath(new URL("./worker.js", import.meta.url)), root, ...args],
...
test("reserve exists at submit boundary; accepted and collected do not claim quality", async () => {
  ...
  try {
    ...
    assert.equal(
      (await x.engine.collect(j.operation_id, signal())).status,
      "needs_review",
    );
...
test("lost receipt survives restart and changing operation cannot bypass semantic identity", async () => {
  ...
  try {
    assert.equal(
      (await x.engine.submit(requestFixture(), familyFixture, signal())).status,
      "submission_unknown",
    );
