...
export class JobOrchestrator {
  ...
  async quote(input: unknown, p: AdaptationPackage, signal: AbortSignal) {
    ...
    const caps = ProviderCapabilitiesSchema.parse(
      await this.provider.capabilities(signal),
    );
    if (
      caps.evidence !== "mock" ||
  ...
  async submit(
    ...
    if (quote.cost.basis !== "mock") throw new Error("Expected mock quote");
    signal.throwIfAborted();
    ...
    try {
      const result = SubmitResultSchema.parse(
        await this.provider.submit(request, signal),
      );
      return this.store.update(
        job.operation_id,
        (j) => {
  ...
  private receipts(j: GenerationJob, receipts: Receipt[]) {
  ...
  async poll(id: string, signal: AbortSignal): Promise<GenerationJob> {
    signal.throwIfAborted();
    ...
    const result = PollResultSchema.parse(
      await this.provider.poll(job.job_id, signal),
    );
    ...
    return this.store.update(
      ...
      (j) => {
        ...
        if (
          ...
          !["needs_review", "succeeded", "failed", "cancelled"].includes(
            j.status,
          )
        )
  ...
  async cancel(
    id: string,
    signal: AbortSignal,
  ): Promise<{ job: GenerationJob; cancel_status: string }> {
    signal.throwIfAborted();
    const job = this.known(id);
    if (!job.job_id) return { job, cancel_status: "unknown_no_job_id" };
    if (
      ["succeeded", "failed", "cancelled", "needs_review"].includes(job.status)
    )
    ...
    const result = CancelResultSchema.parse(
      await this.provider.cancel(job.job_id, signal),
    );
    if (result.job_id !== job.job_id)
      throw new Error("Cancel identity mismatch");
    return {
      job: this.store.update(
        id,
        (j) => {
          this.receipts(j, result.receipts);
          if (
  ...
  async collect(id: string, signal: AbortSignal): Promise<GenerationJob> {
    signal.throwIfAborted();
    const job = this.known(id);
    if (!job.job_id || !["succeeded", "needs_review"].includes(job.status))
      throw new Error("Remote generation is not complete");
    const result = CollectResultSchema.parse(
      await this.provider.collect(job.job_id, signal),
    );
    if (result.job_id !== job.job_id)
