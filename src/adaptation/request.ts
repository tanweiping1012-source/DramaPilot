...
export function buildRequest(
  ...
  operation: string,
  shotId: string,
  task: GenerationRequest["task"] = "video",
): GenerationRequest {
  const plan = p.plans.find((s) => s.shot_id === shotId);
  if (!plan) throw new Error("Unknown shot");
  const request: GenerationRequest = {
    ...
    evidence: "mock",
    input_artifacts: [],
    input_revisions: currentRevisions(p).filter(
      (r) =>
    ...
    parameters: {
      prompt:
        "Offline engineering fixture. No actual source recognition or media quality claim.",
      seed: null,
      audio_mode: "separate",
      ...
      dialogue: plan.line_revisions.map((ref) => {
        ...
        const c = p.characters.find(
          (c) => c.character_id === l.speaker_character_id,
        )!;
        return {
          line_ref: ref,
          speaker_ref: {
