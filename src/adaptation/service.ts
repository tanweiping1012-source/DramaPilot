...
export function saveAdaptation(
  ...
  try {
    ...
    const rows = store.db
      .prepare(
        "SELECT revision,data FROM adaptations ORDER BY revision DESC LIMIT 1",
      )
      .get() as { revision: number; data: string } | undefined;
    if (rows) {
      const old = JSON.parse(rows.data) as AdaptationPackage;
      ...
      if (
        canonicalHash(p.story) !== canonicalHash(old.story) &&
        p.project.revision === old.project.revision
      )
        throw new Error(
          "Source story changed without source revision/re-ingest",
        );
      const check = (a: any, b: any, label: string) => {
        const { revision: ar, ...ac } = a;
        const { revision: br, ...bc } = b;
        if (br < ar) throw new Error(`Decreasing ${label} revision`);
        if (canonicalHash(ac) !== canonicalHash(bc) && br <= ar)
          throw new Error(`Changed ${label} must advance revision`);
      };
      for (const c of p.characters) {
        const prev = old.characters.find(
          (x) => x.character_id === c.character_id,
        );
        if (prev) check(prev, c, "character");
      }
      for (const l of p.lines) {
        const prev = old.lines.find((x) => x.line_id === l.line_id);
        if (prev) check(prev, l, "line");
      }
      ...
      for (const plan of p.plans) {
        ...
        if (prev) check(prev, plan, "shot");
      }
      if (
        canonicalHash(old.changes.map((c) => ({ ...c, approval: null }))) !==
...
export function readAdaptation(
  ...
  const row = (
    revision === undefined
      ? store.db
          .prepare(
            "SELECT data FROM adaptations ORDER BY revision DESC LIMIT 1",
          )
          .get()
      : store.db
  ...
  if (!row) throw new Error("Unknown adaptation");
...
export function invalidatedArtifacts(
  artifacts: Artifact[],
  change: RevisionRef,
  audioMode: "native" | "separate" | "none",
): string[] {
  return artifacts
    .filter((a) => {
      if (["episode", "mix", "report"].includes(a.kind)) return true;
      if (
        !a.input_revisions.some(
          (r) =>
            r.kind === change.kind &&
            r.id === change.id &&
