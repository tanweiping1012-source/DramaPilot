...
import {
  AdaptationPackageSchema,
  GenerationRequestSchema,
  LocalizedEpisodeSchema,
  type AdaptationPackage,
  type RevisionRef,
...
export function currentRevisions(p: AdaptationPackage): RevisionRef[] {
  return [
...
export function assertCurrent(
  ...
  for (const r of refs)
    if (
      !current.some(
        (c) => c.kind === r.kind && c.id === r.id && c.revision === r.revision,
      )
    )
      throw new Error(
        `Missing/stale dependency ${r.kind}:${r.id}@${r.revision}`,
      );
}
...
export function validateAdaptation(input: unknown): AdaptationPackage {
  const p = AdaptationPackageSchema.parse(input),
    refs = currentRevisions(p);
  unique(
    p.characters.map((c) => c.character_id),
    "character",
  );
  unique(
    p.shots.map((s) => s.shot_id),
    "shot",
  );
  unique(
    p.plans.map((s) => s.shot_id),
    "plan",
  );
  unique(
    p.lines.map((l) => l.line_id),
    "line",
  );
  unique(
    p.changes.map((c) => c.change_id),
    "change",
  );
  unique(
    p.story.beats.map((b) => b.beat_id),
    "beat",
  );
  ...
  for (const c of p.story.characters)
    if (!chars.has(c.character_id)) throw new Error("Unknown story character");
  ...
  for (const s of p.shots)
    for (const id of s.character_ids)
      if (!chars.has(id)) throw new Error("Unknown shot character");
  for (const l of p.lines) {
    if (!chars.has(l.speaker_character_id)) throw new Error("Unknown speaker");
    for (const id of l.source_dialogue_ids)
      if (!sources.has(id)) throw new Error("Unknown source dialogue");
  }
  for (const plan of p.plans) {
    const shot = p.shots.find((s) => s.shot_id === plan.shot_id);
    if (!shot) throw new Error("Unknown plan shot");
    if (
      plan.target_character_revisions.some((r) => r.kind !== "character") ||
    ...
    if (
      shot.character_ids.some(
        (id) => !plan.target_character_revisions.some((r) => r.id === id),
      )
    )
    ...
    for (const r of plan.line_revisions) {
      ...
      if (
        !line.source_dialogue_ids.some((id) => shot.dialogue_ids.includes(id))
      )
  ...
  for (const c of p.changes) {
    if (
      c.preserved_beats.some(
        (id) => !p.story.beats.some((b) => b.beat_id === id),
      )
    )
...
export function validateRequest(
  ...
  unique(
    r.parameters.dialogue.map((d) => d.line_ref.id),
    "dialogue line",
  );
  if (
    r.parameters.audio_mode === "native" &&
  ...
  for (const d of r.parameters.dialogue) {
    ...
    const l = p.lines.find((l) => l.line_id === d.line_ref.id),
      c = p.characters.find((c) => c.character_id === d.speaker_ref.id);
    if (
      !l ||
      !c ||
  ...
  if (
    ["video", "tts", "lip_sync"].includes(r.task) &&
    plan.line_revisions.some(
      (ref) => !r.parameters.dialogue.some((d) => d.line_ref.id === ref.id),
    )
  )
  ...
  if (
    r.parameters.driving_audio_artifact_id !== null &&
    !r.input_artifacts.some(
      (a) =>
        a.artifact_id === r.parameters.driving_audio_artifact_id &&
...
export function validateEpisode(input: unknown, p: AdaptationPackage) {
  const e = LocalizedEpisodeSchema.parse(input);
  if (e.revision !== p.revision) throw new Error("Stale episode revision");
  if (
    e.shot_assets.length === 0 ||
    e.resolved_timeline.segments.length !== p.plans.length
  )
    throw new Error("Incomplete episode coverage");
  unique(
    e.resolved_timeline.segments.map((s) => s.shot_id),
    "timeline shot",
  );
  ...
  if (
    ...
    [e.qa_report.visual, e.qa_report.language, e.qa_report.culture].includes(
      "pass",
    )
  )
  ...
  for (const a of [
    ...
    assertCurrent(
      a.input_revisions.filter((r) => r.kind !== "adaptation"),
      currentRevisions(p),
    );
    if (
      a.evidence !== "live" &&
  ...
  for (const s of e.resolved_timeline.segments) {
    ...
    if (!a?.probe) throw new Error("Missing probed video");
    const plan = p.plans.find((p) => p.shot_id === s.shot_id);
    if (!plan) throw new Error("Unknown timeline shot");
