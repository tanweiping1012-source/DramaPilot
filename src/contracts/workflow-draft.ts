...
export const WORKFLOW_DRAFT_VERSION = "0.3.0-draft.1" as const;
...
const ids = z.array(id).superRefine((v, c) => {
  if (new Set(v).size !== v.length) c.addIssue({ code: "custom", message: "Duplicate identity" });
});
...
const refs = z.array(WorkflowRefSchema).superRefine((v, c) => {
  if (new Set(v.map(r => `${r.kind}:${r.id}`)).size !== v.length) c.addIssue({ code: "custom", message: "Duplicate reference identity" });
});
const base = <K extends z.infer<typeof kinds>>(kind: K) => ({ kind: z.literal(kind), id, revision });
...
type Versioned = { kind: z.infer<typeof kinds>; id: string; revision: number };
function fail(message: string): never { throw new Error(message); }
function requireThat(condition: unknown, message: string): asserts condition { if (!condition) fail(message); }
function unique(items: { id: string }[], label: string) { requireThat(new Set(items.map(x => x.id)).size === items.length, `Duplicate ${label} identity`); }
function has(idsToCheck: string[], items: { id: string }[], label: string) { for (const value of idsToCheck) requireThat(items.some(x => x.id === value), `Missing ${label}: ${value}`); }
...
export function workflowHash(value: unknown): string {
  const walk = (v: unknown): unknown => {
    ...
    if (typeof v === "number") { requireThat(Number.isFinite(v), "Non-finite JSON number"); return v; }
    if (Array.isArray(v)) {
...
export function revisionRef(value: Versioned): WorkflowRef { return WorkflowRefSchema.parse({ kind: value.kind, id: value.id, revision: value.revision, sha256: workflowHash(value) }); }
export function assertImmutableRevision(before: Versioned, after: Versioned): void {
  ...
  if (before.revision === after.revision) requireThat(workflowHash(before) === workflowHash(after), "Immutable revision content changed");
}
function match(ref: WorkflowRef, value: Versioned, label: string) { requireThat(ref.kind === value.kind && ref.id === value.id && ref.revision === value.revision && ref.sha256 === workflowHash(value), `Missing/stale/hash-mismatched ${label}`); }
function resolve(ref: WorkflowRef, items: Versioned[], label: string): Versioned {
  const found = items.find(x => x.kind === ref.kind && x.id === ref.id);
  requireThat(found, `Missing ${label}: ${ref.kind}:${ref.id}`); match(ref, found, label); return found;
}
function sourceObjects(p: SourceAssetPackage): Versioned[] { return [p, p.storyboard, ...p.assets, ...p.evidence, ...p.characters, ...p.relations, ...p.dialogue, ...p.scenes, ...p.props, ...p.beats, ...p.storyboard.shots]; }
function targetObjects(t: TargetStoryboard): Versioned[] { return [t, ...t.characters, ...t.scenes, ...t.props, ...t.lines, ...t.shots]; }
export function validateSourceAssetPackage(input: unknown): SourceAssetPackage {
  const p = SourceAssetPackageSchema.parse(input);
  for (const [label, items] of Object.entries({ assets: p.assets, evidence: p.evidence, characters: p.characters, relations: p.relations, dialogue: p.dialogue, scenes: p.scenes, props: p.props, beats: p.beats, shots: p.storyboard.shots })) unique(items, label);
  for (const e of p.evidence) {
    const a = p.assets.find(a => a.id === e.source_asset_id); requireThat(a && a.sha256 === e.source_asset_sha256, "Evidence asset/hash mismatch");
    const l = e.locator;
    if (l.kind === "time") requireThat(a.duration_ms !== null && l.end_ms > l.start_ms && l.end_ms <= a.duration_ms, "Evidence time out of bounds");
    if (l.kind === "frames") requireThat(a.media_kind === "video" && a.frame_count !== null && l.end_frame_exclusive > l.start_frame && l.end_frame_exclusive <= a.frame_count, "Evidence frame out of bounds");
    if (l.kind === "text") requireThat(["script", "subtitles"].includes(a.media_kind) && a.char_count !== null && l.end_char_exclusive > l.start_char && l.end_char_exclusive <= a.char_count, "Invalid text evidence locator");
    if (e.epistemic_status !== "observed") requireThat(e.uncertainties.length > 0, "Inference/reconstruction requires uncertainty");
    if (e.method === "approximate_reconstruction") requireThat(e.epistemic_status === "reconstructed", "Reconstruction cannot be observed fact");
    if (a.evidence === "fixture" || a.evidence === "mock") requireThat(e.evidence === a.evidence, "Synthetic evidence cannot be relabeled live/manual");
    if (e.method === "authored_fixture") requireThat(e.evidence === "fixture", "Fixture method requires fixture evidence");
  }
  for (const x of [...p.characters, ...p.relations, ...p.dialogue, ...p.scenes, ...p.props, ...p.beats, ...p.storyboard.shots]) has(x.evidence_ids, p.evidence, "source evidence");
  for (const r of p.relations) has([r.from_character_id, r.to_character_id], p.characters, "relation character");
  for (const d of p.dialogue) has([d.speaker_character_id], p.characters, "source speaker");
  const lastEnds = new Map<string, number>();
  for (const s of p.storyboard.shots) {
    const a = p.assets.find(a => a.id === s.source_asset_id); requireThat(a && a.sha256 === s.source_asset_sha256 && a.media_kind === "video" && a.duration_ms !== null, "Source shot requires exact video asset");
    requireThat(s.source_out_ms > s.source_in_ms && s.source_out_ms <= a.duration_ms, "Source shot interval out of bounds");
    requireThat(s.source_in_ms >= (lastEnds.get(a.id) ?? 0), "Source storyboard order/overlap invalid"); lastEnds.set(a.id, s.source_out_ms);
    has(s.character_ids, p.characters, "shot character"); has(s.dialogue_ids, p.dialogue, "shot dialogue"); has([s.scene_id], p.scenes, "shot scene"); has(s.prop_ids, p.props, "shot prop");
  }
  for (const b of p.beats) has(b.source_shot_ids, p.storyboard.shots, "beat source shot");
  return p;
}
export function validateTargetStoryboard(input: unknown, sourceInput: unknown): TargetStoryboard {
  const source = validateSourceAssetPackage(sourceInput), t = TargetStoryboardSchema.parse(input); match(t.source_ref, source, "source package");
  for (const [label, items] of Object.entries({ characters: t.characters, scenes: t.scenes, props: t.props, lines: t.lines, shots: t.shots })) unique(items, `target ${label}`);
  for (const c of t.characters) has(c.source_character_ids, source.characters, "source character binding");
  for (const s of t.scenes) has(s.source_scene_ids, source.scenes, "source scene binding");
  for (const p of t.props) has(p.source_prop_ids, source.props, "source prop binding");
  for (const l of t.lines) { has(l.source_dialogue_ids, source.dialogue, "source dialogue binding"); has([l.speaker_character_id], t.characters, "target line speaker"); }
  for (const shot of t.shots) {
    has(shot.source_shot_ids, source.storyboard.shots, "source shot mapping"); has(shot.preserved_beat_ids, source.beats, "source beat mapping");
    for (const r of shot.character_refs) resolve(r, t.characters, "target character");
    resolve(shot.scene_ref, t.scenes, "target scene"); for (const r of shot.prop_refs) resolve(r, t.props, "target prop");
    for (const r of shot.line_refs) { const line = resolve(r, t.lines, "target line") as TargetStoryboard["lines"][number]; requireThat(shot.character_refs.some(c => c.id === line.speaker_character_id), "Shot omits line speaker character"); }
    if (shot.route !== "regenerate") requireThat(shot.source_shot_ids.length > 0, "Edit/reuse requires a source shot mapping");
    if (shot.route === "reuse") requireThat(shot.source_shot_ids.length === 1, "Reuse requires one exact source shot mapping");
    if (shot.audio_mode === "silent") requireThat(shot.line_refs.length === 0, "Silent shot has dialogue");
  }
  unique(t.omitted_beat_decisions.map(d => ({ id: d.source_beat_id })), "omitted beat decision");
  for (const d of t.omitted_beat_decisions) { has([d.source_beat_id], source.beats, "omitted beat"); requireThat(!source.beats.find(b => b.id === d.source_beat_id)!.required, "Required narrative beat cannot be omitted"); requireThat(!t.shots.some(s => s.preserved_beat_ids.includes(d.source_beat_id)), "Beat both preserved and omitted"); }
  for (const b of source.beats) requireThat(t.shots.some(s => s.preserved_beat_ids.includes(b.id)) || t.omitted_beat_decisions.some(d => d.source_beat_id === b.id), `Unaccounted narrative beat: ${b.id}`);
  return t;
}
export function validatePrevisArtifact(input: unknown, context: { source: unknown; target?: unknown }): PrevisArtifact {
  ...
  for (const r of p.input_refs) resolve(r, objects, "previs input");
  const shotObjects = p.domain === "source" ? source.storyboard.shots : target!.shots;
  for (const r of p.shot_refs) resolve(r, shotObjects, "previs shot scope");
  ...
  const expectedObjects: Versioned[] = [...selectedShots];
  if (p.domain === "source") {
    for (const shot of selectedShots as SourceAssetPackage["storyboard"]["shots"]) {
      expectedObjects.push(...source.assets.filter(a => a.id === shot.source_asset_id),
        ...source.characters.filter(c => shot.character_ids.includes(c.id)),
        ...source.scenes.filter(s => s.id === shot.scene_id),
        ...source.props.filter(v => shot.prop_ids.includes(v.id)),
        ...source.evidence.filter(e => shot.evidence_ids.includes(e.id)));
    }
  } else {
    for (const shot of selectedShots as TargetStoryboard["shots"]) {
  ...
  if (p.method === "fixture") requireThat(p.evidence === "fixture", "Fixture previs cannot claim live evidence");
  if (p.method === "reconstructed") requireThat(p.uncertainties.length > 0, "Reconstructed previs needs uncertainty");
  if (p.status === "rendered") requireThat(p.output.evidence === p.evidence, "Previs output evidence mismatch");
  ...
  if (p.domain === "source") requireThat(p.duration_ms === selectedSource[0]!.source_out_ms - selectedSource[0]!.source_in_ms, "Source previs retiming requires a future explicit mapping");
  ...
  const ordered = (points: { at_ms: number }[]) => { let last = -1; for (const point of points) { requireThat(point.at_ms > last && point.at_ms <= p.duration_ms, "Previs keyframes must be ordered and in bounds"); last = point.at_ms; } requireThat(points[0]?.at_ms === 0, "Previs needs an initial state at zero"); };
  for (const t of p.entity_tracks) { resolve(t.entity_ref, entities, "previs entity"); requireThat(entityIds.includes(`${t.entity_ref.kind}:${t.entity_ref.id}`), "Previs entity outside selected shot"); ordered(t.keyframes); }
  for (const entityId of new Set(entityIds)) requireThat(p.entity_tracks.some(t => `${t.entity_ref.kind}:${t.entity_ref.id}` === entityId), `Missing initial entity track: ${entityId}`);
  ordered(p.camera_keyframes);
  unique(p.dialogue_timing.map(t => ({ id: t.line_ref.id })), "previs dialogue timing");
  if (p.domain === "source") requireThat(p.dialogue_timing.length === 0, "Target dialogue timing cannot be stored on source previs");
  const requiredLines = selectedTarget.flatMap(s => s.line_refs);
  for (const timing of p.dialogue_timing) {
    resolve(timing.line_ref, target!.lines, "previs dialogue"); requireThat(requiredLines.some(r => r.id === timing.line_ref.id), "Timing line outside target shot");
    ...
    if (timing.method === "unknown" || timing.read_duration_ms === null) requireThat(timing.assessment === "unknown", "Unknown timing cannot pass");
    else { const overflow = timing.start_ms + timing.read_duration_ms > p.duration_ms; requireThat(timing.assessment === (overflow ? "overflow" : timing.method === "estimate" ? "unknown" : "fits"), "Timing assessment does not match measured interval"); }
    if (timing.method === "test_fixture") requireThat(timing.evidence === "fixture", "Fixture timing cannot claim live evidence");
  }
  if (p.domain === "target") {
    ...
    for (const r of requiredLines) requireThat(p.dialogue_timing.some(t => t.line_ref.id === r.id), "Target dialogue timing missing");
  }
  if (p.overlapping_dialogue === "forbidden") {
    ...
    for (let i = 1; i < measured.length; i++) requireThat(measured[i]!.start_ms >= measured[i - 1]!.start_ms + measured[i - 1]!.read_duration_ms!, "Unapproved overlapping dialogue");
  }
  return p;
}
export function validateProductionPackage(input: unknown, context: { source: unknown; target: unknown; previs: unknown[] }): ProductionPackage {
  ...
  match(p.source_ref, source, "production source"); match(p.target_ref, target, "production target");
  const previs = context.previs.map(v => validatePrevisArtifact(v, { source, target })); unique(previs, "previs");
  for (const r of p.previs_refs) resolve(r, previs, "production previs");
  const selected = p.previs_refs.map(r => previs.find(x => x.id === r.id)!);
  for (const s of target.shots) requireThat(selected.filter(v => v.domain === "target" && v.shot_refs.some(r => r.id === s.id)).length === 1, "Every target shot needs one previs or explicit skip");
  unique(p.audio.map(a => ({ id: a.line_ref.id })), "production audio");
  for (const a of p.audio) resolve(a.line_ref, target.lines, "production audio line");
  unique(p.dialogue_timing.map(t => ({ id: `${t.shot_ref.id}:${t.line_ref.id}` })), "production timing");
  for (const t of p.dialogue_timing) {
    const shot = resolve(t.shot_ref, target.shots, "production timing shot") as TargetStoryboard["shots"][number];
    ...
    if (t.method === "test_fixture") requireThat(t.evidence === "fixture", "Fixture production timing relabeled");
  }
  for (const shot of target.shots) {
    if (shot.overlapping_dialogue === "intentional") continue;
    const intervals = p.dialogue_timing.filter(t => t.shot_ref.id === shot.id && t.read_duration_ms !== null).sort((a, b) => a.start_ms - b.start_ms);
    for (let i = 1; i < intervals.length; i++) requireThat(intervals[i]!.start_ms >= intervals[i - 1]!.start_ms + intervals[i - 1]!.read_duration_ms!, "Unapproved overlapping production dialogue");
  }
  if (p.purpose === "production_candidate") {
    for (const shot of target.shots) for (const lineRef of shot.line_refs) {
    ...
    for (const s of target.shots.filter(s => s.audio_mode === "native_audio")) for (const r of s.line_refs) requireThat(p.audio.some(a => a.line_ref.id === r.id && a.evidence !== "fixture" && a.evidence !== "mock"), "Native audio shot requires exact non-synthetic audio binding");
  }
  return p;
}
/** Binding integrity only. Even a confirmed record conveys zero execution authority. */
export function validateUntrustedConfirmation(input: unknown, context: { stagePlan?: unknown; productionPackage?: unknown }): UntrustedConfirmation {
  ...
  if (c.scope === "preproduction_stage") {
    requireThat(context.stagePlan !== undefined, "Missing stage plan"); const plan = StagePlanSchema.parse(context.stagePlan);
    match(c.stage_plan_ref, plan, "confirmation stage plan"); requireThat(c.stage_input_sha256 === workflowHash(plan.input_refs), "Stage input hash mismatch");
    ...
    requireThat(context.productionPackage !== undefined, "Missing production package"); const p = ProductionPackageSchema.parse(context.productionPackage); match(c.production_package_ref, p, "confirmation production package");
  }
  return c;
}
/** Reports losses; deliberately creates neither a draft package nor an execution grant. */
export function auditLegacyMigration(input: unknown) {
  const legacy = validateAdaptation(input);
  return {
    source_contract_version: "0.2.1" as const, target_contract_version: WORKFLOW_DRAFT_VERSION, legacy_sha256: workflowHash(legacy), lossless: false as const, execution_authorized: false as const,
    ...
    ledger_action: "none" as const,
  };
}
/** An old plan cannot represent splits, merges or new shots without information loss. */
export function assertLegacyShotMappingCompatible(targetInput: unknown, sourceInput: unknown): void {
