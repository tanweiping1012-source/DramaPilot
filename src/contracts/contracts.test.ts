...
import {
  ...
  CostSchema,
  RevisionRefsSchema,
  ResolvedTimelineSchema,
  ShotSchema,
} from "./index.js";
test("original fixture strictly separates facts and proposed US changes", () => {
  assert.equal(
    AdaptationPackageSchema.parse(familyFixture).project.evidence,
    "fixture",
  );
  assert.throws(() =>
    AdaptationPackageSchema.parse({ ...familyFixture, extra: "shell" }),
  );
});
...
test("reject duplicate revisions, overlapping frames and reversed source interval", () => {
  assert.throws(() =>
    RevisionRefsSchema.parse([
      { kind: "line", id: "l1", revision: 1 },
      { kind: "line", id: "l1", revision: 2 },
    ]),
  );
  assert.throws(() =>
    ResolvedTimelineSchema.parse({
      fps_num: 24,
      fps_den: 1,
      segments: [
  ...
  assert.throws(() =>
    ShotSchema.parse({ ...familyFixture.shots[0], source_out_ms: 0 }),
  );
});
