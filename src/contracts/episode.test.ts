...
function episode() {
  ...
  const assets: Artifact[] = familyFixture.plans.map((plan, i) => ({
    artifact_id: `v${i}`,
    kind: "video",
    uri: `/tmp/v${i}`,
    sha256: "0".repeat(64),
    evidence: "fixture",
    input_revisions: refs.filter(
      (r) => r.kind !== "shot" || r.id === plan.shot_id,
    ),
    operation_id: `o${i}`,
...
test("C-001 missing character/line dependency rejected", () => {
  ...
  e.shot_assets[0]!.input_revisions = e.shot_assets[0]!.input_revisions.filter(
    (r) => r.kind !== "character",
  );
