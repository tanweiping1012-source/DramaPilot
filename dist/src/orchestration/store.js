...
export class JobStore {
    ...
    constructor(path, budget) {
        ...
        if (saved.currency !== budget.currency ||
    ...
    budget() {
        return JSON.parse(this.db.prepare("SELECT data FROM config WHERE id=1").get().data);
    }
    get(id) {
        const row = this.db
    ...
    list() {
        return this.db.prepare("SELECT data FROM jobs ORDER BY rowid").all().map((r) => GenerationJobSchema.parse(JSON.parse(r.data)));
    }
    event(id, event, data) {
        this.db
    ...
    exposure(job) {
        const known = job.receipts.reduce((s, r) => s + (r.cost.basis === "unknown" ? 0 : r.cost.amount_micros), 0);
        return job.billing_state === "settled"
            ? known
            : Math.max(job.reserved.amount_micros, known);
    }
    reserve(job, semanticHash) {
        ...
        try {
            ...
            if (this.list().reduce((s, j) => s + this.exposure(j), 0) +
                job.reserved.amount_micros >
                b.amount_micros)
                throw new Error("Budget exhausted including unknown reservations");
            this.db
                .prepare("INSERT INTO jobs VALUES (?,?,?)")
                .run(job.operation_id, semanticHash, JSON.stringify(GenerationJobSchema.parse(job)));
