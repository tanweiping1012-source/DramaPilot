...
import { z } from "zod";
import { SourceAssetPackageSchema, SourceEvidenceSchema, WORKFLOW_DRAFT_VERSION, validateSourceAssetPackage, workflowHash } from "../contracts/workflow-draft.js";
import { renderSourceReport } from "../review/source-report.js";
import { parseSrt } from "./srt.js";
export { parseSrt } from "./srt.js";
export const ASSISTED_INGEST_VERSION = "0.1.0";
...
function ensure(condition, message) { if (!condition)
    throw new Error(message); }
const inside = (base, value) => { const rel = relative(base, value); return rel !== "" && !rel.startsWith(`..${sep}`) && rel !== ".." && !isAbsolute(rel); };
async function checkedPath(root, input) {
...
async function copySnapshot(root, input, destination, limit, signal) {
        ...
        try {
            while (true) {
                ...
                while (written < chunk.length)
                    written += (await output.write(chunk, written, chunk.length - written, null)).bytesWritten;
            }
            await output.sync();
        }
        finally {
...
async function probeVideo(path, executable, signal) {
    signal?.throwIfAborted();
    ...
    const rate = (value) => typeof value === "string" && /^\d+\/\d+$/.test(value) ? value : "unknown";
...
function textBoundary(text, offset) { return !(offset > 0 && offset < text.length && /[\uD800-\uDBFF]/.test(text[offset - 1]) && /[\uDC00-\uDFFF]/.test(text[offset])); }
/** Offline, assisted file import. Annotations remain human assertions, not machine video understanding. */
export async function importAssistedSource(input) {
    ...
    for (const a of manifest.assets)
        if (a.media_kind === "subtitles")
            ensure(manifest.assets.some(v => v.id === a.video_asset_id && v.media_kind === "video"), "Subtitle binding requires a video asset");
    ...
    try {
        ...
        const assets = [], records = [], texts = new Map();
        let totalBytes = 0;
        for (const [index, asset] of manifest.assets.entries()) {
            ...
            ensure(totalBytes <= INGEST_LIMITS.total_bytes, "Combined asset size exceeds import limit");
        ...
        const bindHash = (id) => { const asset = assets.find(a => a.id === id); ensure(asset, `Unknown source asset: ${id}`); return asset.sha256; };
