...
import { SubmitResultSchema, PollResultSchema, CancelResultSchema, CollectResultSchema } from '../contracts/index.js';
...
async function setup(scenario, fn) {
    const dir = await mkdtemp(join(tmpdir(), 'dp-mock space-'));
    try {
        await fn(new MockGenerationProvider({ storeDir: dir, scenario }), dir);
    }
    finally {
        await rm(dir, { recursive: true });
    }
}
async function submit(provider) { const result = SubmitResultSchema.parse(await provider.submit(request(), signal)); assert.equal(result.status, 'accepted'); if (result.status !== 'accepted')
    throw new Error('Missing ID'); return result.job_id; }
async function finish(provider, id) { for (let i = 0; i < 3; i++)
    await provider.poll(id, signal); }
test('quote binds canonical exact request and explicitly simulated zero', async () => setup('success', async (p) => {
    const q = await p.quote(request(), signal);
    assert.equal(q.request_hash, canonicalHash(request()));
    assert.deepEqual(q.cost, { currency: 'USD', amount_micros: 0, basis: 'mock' });
}));
test('queued/running/succeeded and collection use contract; report is never fake video', async () => setup('success', async (p) => {
    ...
    for (const expected of ['queued', 'running', 'succeeded'])
        assert.equal(PollResultSchema.parse(await p.poll(id, signal)).status, expected);
    const collection = CollectResultSchema.parse(await p.collect(id, signal));
    assert.equal(collection.status, 'collected');
    assert.equal(collection.artifacts[0].kind, 'report');
    assert.equal(collection.artifacts[0].evidence, 'mock');
...
test('receipt loss persists remote acceptance across instance restart with no fabricated ID', async () => setup('lost_receipt', async (p, dir) => {
    ...
    assert.equal(diagnostics.submitCount, 1);
...
test('poll timeout survives persistence and recovery polls same ID without submit', async () => setup('poll_timeout', async (p, dir) => {
    const id = await submit(p);
    await assert.rejects(p.poll(id, signal), /MOCK_POLL_TIMEOUT/);
    const restarted = new MockGenerationProvider({ storeDir: dir });
    await finish(restarted, id);
    assert.equal((await restarted.collect(id, signal)).status, 'collected');
    assert.equal((await restarted.diagnostics()).submitCount, 1);
    assert.equal((await restarted.diagnostics()).pollCount, 4);
}));
...
test('failed and expired jobs never cause collection to submit again', async () => {
    for (const scenario of ['failed', 'artifact_expired'])
        await setup(scenario, async (p) => { const id = await submit(p); await finish(p, id); assert.equal((await p.collect(id, signal)).status, scenario === 'failed' ? 'failed' : 'expired'); assert.equal((await p.diagnostics()).submitCount, 1); });
});
test('changed collected bytes are rejected without regeneration', async () => setup('success', async (p) => {
    const id = await submit(p);
    await finish(p, id);
    const a = (await p.collect(id, signal)).artifacts[0];
    await writeFile(fileURLToPath(a.uri), 'corrupted');
    await assert.rejects(p.collect(id, signal), /hash mismatch/);
    assert.equal((await p.diagnostics()).submitCount, 1);
}));
test('raw provider is not idempotent: duplicate submit remains visible to test A safeguards', async () => setup('success', async (p) => {
    const ids = await Promise.all([submit(p), submit(p)]);
    assert.notEqual(ids[0], ids[1]);
    assert.equal((await p.diagnostics()).submitCount, 2);
}));
test('aborted and live inputs do not submit', async () => setup('success', async (p) => {
    const controller = new AbortController();
    controller.abort();
    await assert.rejects(p.submit(request(), controller.signal));
    await assert.rejects(p.submit({ ...request(), evidence: 'live' }, signal), /only fixture/);
    assert.equal((await p.diagnostics()).submitCount, 0);
}));
