...
if (!manifestFile || !workspace || !output || process.argv.length !== 5)
    throw new Error('Usage: ingest-source <manifest.json> <workspace-root> <new-output-dir>');
...
try {
    ...
    try {
        ...
        if (!s.isFile() || s.size > INGEST_LIMITS.manifest_bytes || s.size === 0)
            throw new Error('Manifest must be a bounded regular JSON file');
        const b = Buffer.alloc(INGEST_LIMITS.manifest_bytes + 1);
        let n = 0;
        while (n < b.length) {
        ...
        if (n > INGEST_LIMITS.manifest_bytes)
