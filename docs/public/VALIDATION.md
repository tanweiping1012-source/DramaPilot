...
公开仓库首轮复跑与 Linux CI 为98 pass / 1 fail / 3 not_run：验收脚本误将三个未分发的内部交接Markdown纳入冻结hash清单，因缺文件而失败。修订仅移除这三个文档条目，全部原TypeScript源码及锁文件hash、行为检查和失败标准保持不变。独立验收者将修订脚本指向公开快照复跑，结果99 pass / 0 fail / 3 not_run；原失败记录保留，不改写为通过。
