...
所有schema拒绝未声明字段。结构校验能检查记录是否自洽，**不能核实文件真的存在、演员身份是否识别正确、文化改编是否自然或用户确实做过确认**。这些工作属于后续导入、文件验证、可信host授权与人工审阅。
...
下一步DP-N2会接真实本地文件与人工剧本/SRT/镜头资料；DP-N3接入独立目标分镜的保存和制作请求；DP-N4才渲染预演。自动识片、真实生成和自动3D仍为后续范围。
